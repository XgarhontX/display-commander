...
- **Proxy loading**: Rename the built addon (`zzz_display_commander.addon64` / `.addon32`) next to the game to one of these proxy DLL names: **dxgi.dll**, **d3d9.dll**, **d3d11.dll**, **d3d12.dll**, **ddraw.dll**, **hid.dll**, **version.dll**, **opengl32.dll**, **dbghelp.dll**, **vulkan-1.dll**, or **winmm.dll**.
  **In the addon (DLL) folder or `%LocalAppData%\Programs\Display_Commander\`:**
  - `.DC_CONFIG_GLOBAL` — store config and ReShade data in `%LocalAppData%\Programs\Display_Commander\Games\<game_name>\` (global per-game folder; `game_name` skips generic exe path segments like `Client` / `Binaries` / `Win64`). The empty flag file can live next to the addon **or** in the Display_Commander app data root (one place is enough). On startup, the addon reads/writes **`.display_commander_installer_marker.json`** in the game install root. Installers may set **`display_commander_config_directory`** there (absolute UTF-8 path) to override the default `Games\<game_name>` folder before launch. Display Commander refreshes the marker each run and includes extra fields: `display_commander_game_install_root`, `display_commander_games_folder_name`, and `display_commander_config_path_source` (`default` or `marker`).
