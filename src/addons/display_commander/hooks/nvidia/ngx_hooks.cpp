...
static void ApplyDLSSPresetParameters(NVSDK_NGX_Parameter* InParameters) {
    ...
    const NVSDK_NGX_PerfQuality_Value quality_preset =
    ...
    if (!preset_override_enabled && static_cast<int>(quality_preset) < 0) {
        return;
    }
    ...
    if (static_cast<int>(quality_preset) >= 0 && NVSDK_NGX_Parameter_SetI_Original != nullptr) {
        const int quality_preset_int = static_cast<int>(quality_preset);
        NVSDK_NGX_Parameter_SetI_Original(InParameters, NVSDK_NGX_Parameter_PerfQualityValue, quality_preset_int);
        g_ngx_parameters.update_int(NVSDK_NGX_Parameter_PerfQualityValue, quality_preset_int);
        LogInfo("Applied DLSS Quality Preset (PerfQualityValue) -> %d", quality_preset_int);
    }
        ...
        if (res == NVSDK_NGX_Result_Success && OutValue != nullptr && InName != nullptr) {
            ...
            if (strcmp(InName, NVSDK_NGX_Parameter_PerfQualityValue) == 0) {
                const NVSDK_NGX_PerfQuality_Value quality_preset =
                    GetDLSSQualityPresetValue(settings::g_swapchainTabSettings.dlss_quality_preset_override.GetValue());
                const int quality_preset_int = static_cast<int>(quality_preset);
                if (quality_preset_int >= 0) {
                    *OutValue = quality_preset_int;
                    NVSDK_NGX_Parameter_SetI_Detour(InParameter, InName, *OutValue);
                    res = NVSDK_NGX_Result_Success;
                }
            }
    ...
    if (NVSDK_NGX_Parameter_GetUI_Original != nullptr) {
        ...
        {  // Override DLSS / Ray Reconstruction render preset GetUI when preset override is enabled
            ...
            if (strcmp(InName, NVSDK_NGX_Parameter_PerfQualityValue) == 0) {
                ...
                const int quality_preset_int = static_cast<int>(quality_preset);
                if (quality_preset_int >= 0) {
                    *OutValue = static_cast<unsigned int>(quality_preset_int);
                    NVSDK_NGX_Parameter_SetUI_Detour(InParameter, InName, *OutValue);
                    res = NVSDK_NGX_Result_Success;
                }
            }
    ...
    if (NVSDK_NGX_Parameter_GetULL_Original != nullptr) {
         ...
         }*/
        {
            ...
            if (strcmp(InName, NVSDK_NGX_Parameter_PerfQualityValue) == 0) {
                ...
                const int quality_preset_int = static_cast<int>(quality_preset);
                if (quality_preset_int >= 0) {
                    *OutValue = static_cast<unsigned long long>(quality_preset_int);
                    NVSDK_NGX_Parameter_SetULL_Detour(InParameter, InName, *OutValue);
                    res = NVSDK_NGX_Result_Success;
                }
            }
...
static const NGXHookEntry kNGXHooks[kNGXHookCount] = {
    {.name = "NVSDK_NGX_D3D12_Init",
     ...
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D12_Init_Original)},
    {.name = "NVSDK_NGX_D3D12_Init_Ext",
    ...
    {.name = "NVSDK_NGX_D3D12_CreateFeature",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D12_CreateFeature_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D12_CreateFeature_Original)},
    {.name = "NVSDK_NGX_D3D12_ReleaseFeature",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D12_ReleaseFeature_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D12_ReleaseFeature_Original)},
    {.name = "NVSDK_NGX_D3D12_EvaluateFeature",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D12_EvaluateFeature_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D12_EvaluateFeature_Original)},
    {.name = "NVSDK_NGX_D3D12_EvaluateFeature_C",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D12_EvaluateFeature_C_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D12_EvaluateFeature_C_Original)},
    {.name = "NVSDK_NGX_D3D11_Init",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D11_Init_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D11_Init_Original)},
    {.name = "NVSDK_NGX_D3D11_Init_Ext",
    ...
    {.name = "NVSDK_NGX_D3D11_CreateFeature",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D11_CreateFeature_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D11_CreateFeature_Original)},
    {.name = "NVSDK_NGX_D3D11_ReleaseFeature",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D11_ReleaseFeature_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D11_ReleaseFeature_Original)},
    {.name = "NVSDK_NGX_D3D11_EvaluateFeature",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D11_EvaluateFeature_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D11_EvaluateFeature_Original)},
    {.name = "NVSDK_NGX_D3D12_GetParameters",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D12_GetParameters_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D12_GetParameters_Original)},
    {.name = "NVSDK_NGX_D3D12_GetCapabilityParameters",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D12_GetCapabilityParameters_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D12_GetCapabilityParameters_Original)},
    {.name = "NVSDK_NGX_D3D12_AllocateParameters",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D12_AllocateParameters_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D12_AllocateParameters_Original)},
    {.name = "NVSDK_NGX_D3D11_GetParameters",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D11_GetParameters_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D11_GetParameters_Original)},
    {.name = "NVSDK_NGX_D3D11_GetCapabilityParameters",
     .detour = reinterpret_cast<LPVOID>(&NVSDK_NGX_D3D11_GetCapabilityParameters_Detour),
     .original = reinterpret_cast<LPVOID*>(&NVSDK_NGX_D3D11_GetCapabilityParameters_Original)},
